...
module GHC.Rename.Env (
        ...
        lookupSyntaxName, lookupSyntaxName_maybe,
        lookupIfThenElse,
...
lookupSyntaxName_maybe :: OccName -> RnM (Maybe Name)
lookupSyntaxName_maybe occ
  = do { rebind <- xoptM LangExt.RebindableSyntax
       ; if rebind
         then Just <$> lookupOccRn (mkRdrUnqual occ)
         else pure Nothing }
...
lookupSyntaxName std_name
  = do { mb_nm <- lookupSyntaxName_maybe (nameOccName std_name)
       ; case mb_nm of
           Nothing -> return (std_name, emptyFVs)
           Just nm -> return (nm, unitFV nm) }
